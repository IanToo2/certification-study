...
| 비동기 메시지 처리 | `Amazon SQS` |
| 팬아웃 | `Amazon SNS` |
| 이벤트 라우팅 | `Amazon EventBridge` |
| 공유 파일 시스템 | `Amazon EFS` |
| 블록 스토리지 | `Amazon EBS` |
| 초저지연 NoSQL | `Amazon DynamoDB` |
| 고정 IP 로드 밸런서 | `Network Load Balancer` 또는 `AWS Global Accelerator` |
...
- 검색할 때는 문제 문장 그대로 찾기보다 `요구사항 핵심어 + 서비스명` 조합으로 찾는 편이 빠르다.
