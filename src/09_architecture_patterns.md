...
- 앞단은 `Route 53 + CloudFront`로 글로벌 진입과 캐시를 담당한다.
- 애플리케이션 계층은 무상태로 두고 `ALB + Auto Scaling`으로 확장한다.
- 데이터 계층은 `RDS Multi-AZ` 또는 `Aurora`로 보호하는 것이 기본 정답 패턴이다.
...
- 진입점은 보통 `API Gateway`, 계산은 `Lambda`, 데이터는 `DynamoDB` 또는 `S3`로 간다.
- 동기 요청과 후속 비동기 작업을 분리해야 서버리스 구조가 안정적이다.
- "운영 부담 최소화"와 "간헐적 트래픽"이 동시에 보이면 먼저 떠올릴 조합이다.
...
- 이벤트 생산자와 소비자를 직접 묶지 않고 `EventBridge`에서 한 번 분기한다.
- 재시도와 버퍼링이 필요하면 `SQS`, 다중 구독은 `SNS`로 보완한다.
- 문제 문장에서 "팬아웃", "규칙 기반 분기", "결합도 감소"가 보이면 이 패턴이다.
...
- 공통 진입점은 `ALB` 또는 API 계층으로 두고 내부 서비스는 분리한다.
- 서비스별 저장소를 독립적으로 선택해야 배포와 확장이 쉬워진다.
- 동기 호출보다 이벤트 전달 비중이 커질수록 마이크로서비스 문제일 가능성이 높다.
...
- 읽기 부하는 먼저 캐시로 줄이고, 그래도 부족하면 `Read Replica`를 추가한다.
- 쓰기 일관성이 중요한 요청은 Primary에 남겨 둬야 한다.
- "고가용성"과 "읽기 확장"은 다른 요구사항이라는 점을 항상 분리해서 본다.
...
- 정적 자산은 `CloudFront + S3`, 동적 요청은 `CloudFront + ALB` 조합으로 처리한다.
- 오리진 부하를 줄이고 전송 지연을 낮추는 것이 핵심이다.
- "글로벌 저지연"이 보여도 캐시가 핵심인지, 네트워크 경로가 핵심인지 먼저 구분한다.
...
- 저장소 중심은 거의 항상 `Amazon S3`다.
- 원본과 가공 데이터를 분리해야 분석과 보안 정책 적용이 쉬워진다.
- 문제 문장에 "대용량 원천 저장", "유연한 분석", "저비용 장기 보관"이 함께 보이면 데이터 레이크다.
...
- 인터넷 공개 구간은 최소화하고 앱과 DB는 프라이빗으로 둔다.
- AWS 서비스 접근도 `VPC Endpoints`로 우회해 공용 인터넷을 줄인다.
- "보안 강화" 문제는 네트워크 구조, 권한, 암호화, 감사 로그를 함께 봐야 한다.
